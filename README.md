...
`mechStatus`が施錠中から解錠中へ変化したときは既存のLIGHT家電をONにし、`nature_unlock_signal_ids`の赤外線信号も送ります。解錠中から施錠中へ変化したときは`nature_lock_signal_ids`の信号を送ります。初回状態や同じ状態の重複通知ではNature Remoを操作しません。
...
解錠時と施錠時に送るsignal IDを配列で設定します。不要な側は空配列のままにします。
...
| `nature_unlock_signal_ids` | 施錠から解錠へ変化したときに送る赤外線signal IDの配列 |
| `nature_lock_signal_ids` | 解錠から施錠へ変化したときに送る赤外線signal IDの配列 |
