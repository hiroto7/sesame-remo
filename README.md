...
Sesame5をBLEで監視するmacOS向けCLIです。Touch Pro解錠でNature Remo照明をONにするモードと、施錠・解錠状態を監視して音声などを制御するモードがあります。Wi-Fiモジュールは不要です。
...
Touch Pro解錠だけを判定したい場合は`touch-pro-trigger`、操作元を区別せず現在の施錠状態を扱いたい場合は`lock-state-monitor`を使います。1回だけ状態や履歴を確認するコマンドは後述の[確認用コマンド](#確認用コマンド)にまとめています。
...
`touch-pro-trigger`は、Sesame5の履歴payloadに含まれる次の差分を使ってTouch Pro経由か判定します。
...
Sesame5は履歴キューの先頭1件を返し、そのレコードを削除するまで次の履歴へ進みません。そのため、`touch-pro-trigger`は処理に成功した履歴をSesame5から削除します。削除しない設定へ単純に変えると、同じ履歴を読み続けて新しいTouch Pro解錠へ到達できません。

この削除には次の副作用があります。

- `touch-pro-trigger`が先に削除した履歴は、公式Sesameアプリが後から取得できない可能性がある
- 公式アプリが先に取得・削除した履歴は、`touch-pro-trigger`が検出できず照明が点かない可能性がある
- 公式アプリと`touch-pro-trigger`が同時にBLE接続すると、一時的な接続失敗や遅延が起こり得る
...
解錠中だけ音声を再生する場合は、`lock-state-monitor`を使います。デフォルトの音源はmacOS内蔵の短い効果音`/System/Library/Sounds/Ping.aiff`です。これは長いBGMではなく、短い音を`afplay`で繰り返し起動する方式なので、連続した警告音やBGMとして使う場合は、実際に音が鳴っている時間が長い音声ファイルを`--sound`で指定してください。`--repeat-gap 0`でも完全なシームレスループは保証されません。

```bash
uv run sesame-remo lock-state-monitor \
  --config config.toml \
  --sound /path/to/continuous-warning.mp3 \
  --repeat-gap 0
```
...
音量、繰り返し間隔、音源はオプションで変更できます。音声ファイルはリポジトリには同梱していません。たとえば次の例では、macOS内蔵の別の短い効果音を指定しています。
...
実機検証で使った音源や、状態通知・再接続・音声停止についての実測結果は[docs/field-verification.md](docs/field-verification.md)にまとめています。
...
同梱plistは`touch-pro-trigger`用です。`lock-state-monitor`を常駐させる場合は、plist内の`touch-pro-trigger`を`lock-state-monitor`へ変更し、必要に応じて`--sound`などの引数を追加してください。
...
| `delete_history_after_read` | `touch-pro-trigger`では`true`必須 |
