...
- 通常の接続監視では、BLEクライアントの実際の切断を検出した場合だけ、対象Sesameの次の広告を使って再接続します。通知の復号・解析失敗は別途1回だけ再接続します。
- 同じMac上で公式Sesameアプリを接続すると、macOSが既存BLE接続を共有して通信セッションが競合する場合があります。`session_replaced`を検知した場合は再接続せず正常終了します。
...
`service install`で生成するplistは、予期しない異常終了だけをLaunchAgentで再起動します。別クライアントによるSesameセッション置換を検知した場合は、安全のため終了コード0で停止するので自動再起動しません。公式Sesameアプリを終了してから、次のコマンドで明示的に再開してください。

```bash
launchctl kickstart "gui/$(id -u)/com.example.sesame-remo"
```
...
- `sesame_protocol_error`の`reason`が`session_replaced`の場合、接続中に同じMac上の別クライアントがSesameの通信セッションを更新した可能性がある。公式Sesameアプリとの再認証競合を避けるため、`monitor_stopped`を出して正常終了する
- `notification_processing_failed`は通知の復号または解析に失敗したことを示す。1回だけ再接続し、正常な状態通知より先に再発した場合は`monitor_stopped`を出して正常終了する
- プロトコルエラーのログには例外の型だけを記録し、秘密鍵、トークン、通知ペイロードは出力しない
- `monitor_stopped`後は公式Sesameアプリを終了し、foregroundなら`sesame-remo monitor`を再実行、LaunchAgentなら`launchctl kickstart`で再開する
