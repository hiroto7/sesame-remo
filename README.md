...
解錠中だけ音声を再生する場合は、`status-daemon`を使います。デフォルトの音源はmacOS内蔵の短い効果音`/System/Library/Sounds/Ping.aiff`です。これは長いBGMではなく、短い音を`afplay`で繰り返し起動する方式なので、連続した警告音やBGMとして使う場合は、実際に音が鳴っている時間が長い音声ファイルを`--sound`で指定してください。`--repeat-gap 0`でも完全なシームレスループは保証されません。
