...
`mechStatus`が施錠中から解錠中へ変化したときは既存のLIGHT家電をONにし、`nature_unlock_signal_ids`の赤外線信号も配列順に送ります。解錠中から施錠中へ変化したときは`nature_lock_signal_ids`の信号を配列順に送ります。初回状態や同じ状態の重複通知ではNature Remoを操作しません。
...
カスタム赤外線家電のボタンを連動させる場合は、次のコマンドでsignal IDを確認できます。

```bash
export NATURE_TOKEN="$(pbpaste)"
curl -sS -H "Authorization: Bearer $NATURE_TOKEN" \
  https://api.nature.global/1/appliances \
  | uv run python -c 'import json, sys; [print(a["nickname"], s["name"], s["id"]) for a in json.load(sys.stdin) for s in a.get("signals", [])]'
unset NATURE_TOKEN
```

解錠時と施錠時に送るsignal IDを配列で設定します。信号は配列の先頭から順に、前のAPI要求が完了してから次を送ります。不要な側は空配列のままにします。
...
| `nature_unlock_signal_ids` | 施錠から解錠へ変化したときに配列順で送る赤外線signal ID |
| `nature_lock_signal_ids` | 解錠から施錠へ変化したときに配列順で送る赤外線signal ID |
...
- `nature_unlock_signal_ids`と`nature_lock_signal_ids`が対象ボタンのsignal IDか確認する
