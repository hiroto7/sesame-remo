...
- Sesame5の履歴は先頭のレコードを削除するまで次へ進まないため、処理成功後の履歴削除が必要です。
...
| `nature_token` | `touch-pro-trigger`用のNature Cloud API Personal Access Token。`lock-state-monitor`では不要 |
| `nature_light_appliance_id` | `touch-pro-trigger`用のNature Remo LIGHT家電ID。`lock-state-monitor`では不要 |
| `nature_light_button` | `touch-pro-trigger`が送るLIGHTボタン。通常は`on` |
