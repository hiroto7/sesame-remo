...
履歴を読まないため、`lock-state-monitor`による履歴削除は発生しません。`touch-pro-trigger`は同じ常時接続・再接続基盤を使いながら、接続中に履歴キューを継続取得し、Touch Proのsource tagを判定します。`combined-monitor`はこの状態通知と履歴通知を1つの認証済みBLE接続で同時に実行します。したがって、音声用途とTouch Pro照明用途を有効にするために、2つのSesame接続を起動する必要はありません。

## 履歴監視の起点

Sesameの広告に含まれる`has_history`ビットは、履歴取得要求を送ってよいかを判断するための起点として使います。履歴payloadそのものは広告に含まれないため、次の順序になります。

1. BLE広告を受信し、`has_history`を確認する
2. 既存の認証済みBLE接続で履歴を1件要求する
3. ハンドラが成功した場合に先頭履歴を削除する
4. 次の`has_history`広告を待つ

`has_history`がない状態で履歴要求を定期実行すると、履歴がない場合でもSesame本体側の通信表示が発生し、無駄なBLE通信になります。現在の実装は履歴あり広告をキューに保持し、広告を受信していない間は履歴要求を送りません。同じ広告が短時間に複数回届いても、待機シグナルは1件にまとめます。

これは公開SDKのソースだけから導けた仕様ではなく、実機で空の履歴要求がSesame本体のランプを点灯させること、および履歴あり広告を起点にすれば新規履歴の監視ができることを確認した上で採用した実装方針です。
...
実機でしか確認できなかった挙動は、別の[実機検証記録](field-verification.md)に分離しています。特に、`mechStatus`通知の間隔を固定値とみなせないこと、旧実装の15秒監視が再接続ループを起こしたこと、空の履歴要求でSesame本体のランプが点灯すること、音声停止時に`afplay`を終了する必要があることは、ソース調査だけでは確定できず実機ログで確認した事項です。
